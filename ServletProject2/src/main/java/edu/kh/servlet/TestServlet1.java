...
public class TestServlet1 extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//-> 요청 시 전달받은 값 중 name 속성 값이 "inputName"인 input 태그의 value를 얻어옴
		...
		//2.만들어진 응답 화면을 전달 받을 클라이언트와의 연결 통로(스트림)얻어오기
		 PrintWriter out = resp.getWriter(); //클라이언트와 연결된 문자기반 스트릠 얻어오기
		 
		 //3. html코드를 작성하여 스트리믈 이용해 출력 
		 ...
		 out.print("<p>");
		 	if(hobby == null) { //선택한 취미가 없을 경우
		 		...
		 		for(String h : hobby) {
		 			out.print(h + "<br>"); //취미 한 줄씩 추가하기
		 		}
		 	}
		 out.print("</p>");
		 
		 out.print("<h4 style='color:yellow'>");
